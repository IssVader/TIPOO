...
class Action(db.Model):
	...
	def count_events_between_dates(self,startDate,endDate,event,user):				
		query = "SELECT * FROM Action WHERE event=:event  AND user=:user AND recorded >= :startDate AND recorded <= :endDate"
		cont = db.GqlQuery(query,event=event,user=user,startDate=startDate,endDate=endDate).fetch(limit = 1000)		
		return len(cont)
